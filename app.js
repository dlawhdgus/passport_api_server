...
MongoClient.connect(config.MONGODB_CONNECT_STRING, function (err, db) {
    ...
    app.get('/board/:id', (req, res) => {
        try {
            ...
            db_article.findOne({ _id: ObjectId(id) }, (err, result) => {
    ...
    app.patch('/board/:id', (req, res) => {
        try {
            ...
            db_article.updateOne({ _id: ObjectId(id) }, updateQuery, (err, result) => {
    ...
    app.delete('/board/:id', (req, res) => {
        try {
            ...
            if (id.length !== 24) res.status(400).send('Bad Request')
            db_article.deleteOne({ _id: ObjectId(id) }, (err, result) => {
    ...
    app.delete('/board', (req, res) => {
        try {
            ...
            db_article.deleteMany(filter, (err, result) => {
                if (result.deletedCount === 0) res.status(404).send('Not Found')
                else res.send('success')
            })
        } catch (error) {
